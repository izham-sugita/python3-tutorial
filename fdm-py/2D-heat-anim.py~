...
for iter in range(itermax):
    ...
    if mod > 0:
        ...
        for i in range(1,imax-1):
            for j in range(1,jmax-1):
                fx[i,j] = fxnew[i,j] + coeff*(fxnew[i,j-1]
                                              -2.0*fxnew[i,j]
                                              +fxnew[i,j+1])
                                              
    else:
        #print("Even number %d"%iter)
         for i in range(1,imax-1):
            for j in range(1,jmax-1):
                fxnew[i,j] = fx[i,j] + coeff*(fx[i,j-1]
                                              -2.0*fx[i,j]
                                              +fx[i,j+1])

         for i in range(1,imax-1):
            for j in range(1,jmax-1):
                fx[i,j] = fxnew[i,j] + coeff*(fxnew[i-1,j]
                                              -2.0*fxnew[i,j]
                                              + fxnew[i+1,j])
